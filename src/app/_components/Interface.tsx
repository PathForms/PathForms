...
const Interface = () => {
  ...
  const handleMove = (direction: Direction) => {
    ...
    if (moves.length > 0 && moves[moves.length - 1] === oppositeMoves[direction]) {
  ...
  const invertPath = (index: number) => {
    if (moveRecords[index]) {
      ...
      setNodePaths((prev) => {
        ...
        return [...prev.slice(0, index), invertedNodes, ...prev.slice(index + 1)];
      });
      setEdgePaths((prev) => {
        if (prev.length === 0) return prev;
        return [...prev.slice(0, index), invertedEdges, ...prev.slice(index + 1)];
      });
      setMoveRecords((prev) => {
        if (prev.length === 0) return prev;
        return [...prev.slice(0, index), invertedMoves, ...prev.slice(index + 1)];
      });
    } else {
